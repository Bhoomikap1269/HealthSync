...
class Receipt(Resource):
    def post(self, id):
        ...
        if appointment:
            ...
            patient = conn.execute("SELECT pat_name FROM patient WHERE pat_id=?", (patient_id,)).fetchone()
            doctor = conn.execute("SELECT doc_name FROM doctor WHERE doc_id=?", (doctor_id,)).fetchone()
            ...
            if patient and doctor:
                ...
                # Generate receipt
                pdf_path = generate_receipt(patient_name, doctor_name, appointment_date)
